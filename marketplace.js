...
onclick="handleBorrow(${c.id},'${encodeURIComponent(c.name)}',${c.tempPrice})">
...
// ── TEMPORARY: check login → payment page ──
function handleBorrow(id, name, tempPrice) {
  const loggedIn = localStorage.getItem('ece_logged_in');
  const dest = `payment.html?id=${id}&name=${name}&type=temporary&amount=${tempPrice}`;
